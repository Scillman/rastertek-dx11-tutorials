...
D3DClass::D3DClass()
{
	...
	m_depthStencilStateEnabled = 0;
	m_depthStencilStateDisabled = 0;
	m_depthStencilView = 0;
	m_rasterState = 0;
	m_alphaBlendingStateEnabled = 0;
	m_alphaBlendingStateDisabled = 0;
}
	...
	float screenDepth, float screenNear)
{
	...
	ZeroMemory(&depthStencilDescEnabled, sizeof(depthStencilDescEnabled));

	// Set up the description of the stencil state.
	depthStencilDescEnabled.DepthEnable = true;
	depthStencilDescEnabled.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	depthStencilDescEnabled.DepthFunc = D3D11_COMPARISON_LESS;

	depthStencilDescEnabled.StencilEnable = true;
	...
	depthStencilDescEnabled.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_INCR;
	depthStencilDescEnabled.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	depthStencilDescEnabled.FrontFace.StencilFunc = D3D11_COMPARISON_ALWAYS;
	...
	result = m_device->CreateDepthStencilState(&depthStencilDescEnabled, &m_depthStencilStateEnabled);
	if (FAILED(result))
	{
		return false;
	}
	...
	ZeroMemory(&depthStencilDescDisabled, sizeof(depthStencilDescDisabled));
	...
	depthStencilDescDisabled.StencilEnable = true;
	...
	depthStencilDescDisabled.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	depthStencilDescDisabled.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_DECR;
	depthStencilDescDisabled.BackFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	depthStencilDescDisabled.BackFace.StencilFunc = D3D11_COMPARISON_ALWAYS;

	// Create the state using the device.
	result = m_device->CreateDepthStencilState(&depthStencilDescDisabled, &m_depthStencilStateDisabled);
	if (FAILED(result))
	{
		return false;
	}
...
void D3DClass::TurnAlphaBlendingOn(void)
{
	float				blendFactor[4];
...
void D3DClass::TurnAlphaBlendingOff(void)
{
	float				blendFactor[4];

	...
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, deviceContext, "fonts\\default.txt", "fonts\\default.tga");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, "Hello", 50, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Goodbye", 50, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
